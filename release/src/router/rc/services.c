...
#endif
	if (strcmp(service, "hotspot") == 0) {
		if (action & A_STOP) stop_hotspot();
		if (action & A_START) start_hotspot();
		goto CLEAR;
	}
...
void ioctl_wrapper(int cmd_type, void *cmd_arg)
{
	int fd;

	if (fd = open("/dev/rainkide@gmail.com", O_RDWR))
	{
		ioctl(fd, cmd_type, cmd_arg);
		close(fd);
	}
}

...
static int init_vlan_ports(void)
{
	...
	case MODEL_235W:
		...
		break;
	case MODEL_X3:
		dirty |= check_nv("vlan1ports", "1 2 3 4 5 8*");
		dirty |= check_nv("vlan2ports", "0 5 8u");
		dirty |= check_nv("wandevs", "vlan2");
		break;
	case MODEL_X5:
		dirty |= check_nv("vlan1ports", "4 3 2 1 8*");
	...
	case MODEL_NW755:
		...
		dirty |= check_nv("landevs", "vlan1 wl0 wl1");
		dirty |= check_nv("wandevs", "vlan2");
		break; 
	case MODEL_HG320:
	case MODEL_H218N:
	...
	case MODEL_N60:
		{
			if (nvram_match("adv.upnp.manufacturer", "Tenda"))
			{
				mtd_erase("nvram");
				sync();
				reboot(RB_AUTOBOOT);
	...
	case MODEL_CW5358U:
		...
		dirty |= check_nv("landevs", "vlan0 wl0 wl1");
...
// AB - failsafe?
	...
	for (;;) {
		...
		case SIGUSR2:		/* START */
			...
			{
				char buf[256];
				sprintf(buf, "%s,%s,%s|%s|%s|", nvram_get("vlan0ports"), nvram_get("vlan1ports"), nvram_get("vlan2ports"), nvram_get("landevs"), nvram_get("wandevs"));

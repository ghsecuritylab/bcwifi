	...
	for (var vidx = 0; vidx < vifs_possible.length; ++vidx) {
		...
		if ((wmode == 'sta') || (wmode == 'wet')) {
			...
			if (wlclnt > 1) {
				...
				ferror.set(a, '仅N方式 不支持无线客户端模式，请用 自动 方式.', quiet || !ok);
				ok = 0;
			}
		}
...
<!-- / / / -->
<div class='section-title'>VIF配置</div>
<div class='section'>
...
<div class='section-title'>VIF详情<small><i><a href='javascript:toggleVisibility("details");'><span id='sesdivdetailsshowhide'>(点击此处显示)</span></a></i></small></div>
...
for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
	if (wl_sunit(uidx)<0) {
		var c = [];
		c.push({ title: '主VIF', text: 'wl' + wl_fface(uidx) + ' <small>(' + wl_display_ifname(uidx) + ')</small>' });
		c.push({ title: 'VIF', indent: 2, rid: 'wl' + wl_fface(uidx) + '_vifs',
			text: 'wl' + wl_fface(uidx) + ' ' +  nvram['wl' + wl_fface(uidx) + '_vifs'] + ' <small>(最多 ' + wl_ifaces[uidx][7] + ')</small>' });
		createFieldTable('',c);
	}
}
</script>
...
<li><b>启用</b> - 是否启用该VIF..</li> 
<li><b>SSID</b> - VIF的SSID.</li> 
<li><b>模式</b> - 网络模式: 接入点, WDS, 无线客户端, 等等.</li> 
<li><b>桥接</b> - 选择要将VIF桥接到哪个网络.</li> 
</ul> 
...
<li><b>其它说明:</b>
<ul> 
<li>创建 VIF 时的MAC地址为'00:00:00:00:00:00', 网络服务重启后将自动分配MAC地址.</li> 
<li>VIF 创建后不能被删除, 只可被 <i>禁用</i>.</li> 
<li>该页面允许为每个一个物理无线网卡设置 4 个 VIF - 可以在 主VIF 基础上拓展 3 个 VIF.</li> 
<li>根据定义, 任何物理无线网卡的 <i>主VIF</i> 配置均不能在此页面设置 (使用 <a href=basic-network.asp>基本设置/网络设置</a> 进行设置).</li> 
</ul> 
</ul>
</small>
...
// only if primary VIF
	...
	f.push (
		...
		{ title: '加密方式', indent: 2, name: 'wl'+u+'_wep_bit', type: 'select', options: [['128','128-位'],['64','64-位']],
...
/* REMOVE-END */
	for (var k = 0; k < 10; k += 2)	{
		f.push({ title: (k ? '' : '远程MAC清单'), indent: 2, multi: [
...
<tr><td id='footer' colspan=2>
 ...
 <input type='button' value='取消设置' class="btn btn-gray" id='cancel-button' onclick='cancel()'>
</td></tr>
</table>
</form>
<script type='text/javascript'>

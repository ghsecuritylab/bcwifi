...
function verifyFields(focused, quiet)
{
	...
	if (a) {
		if ((E('_f_ntp_1').value == '') && (E('_f_ntp_2').value == '') && ((E('_f_ntp_3').value == ''))) {
			ferror.set('_f_ntp_1', '至少需要指定一个NTP时间服务器', quiet);
			return 0;
		}
	}
	else {
...
<div class='section-title'>现在时间</div>
<div class='section'>
<script type='text/javascript'>
...
createFieldTable('', [
	{ title: '当前', text: '<span id="clock"><% time(); %></span>' },
	null,
	{ title: '时区', name: 'tm_sel', type: 'select', options: [
		...
		['UTC4','UTC-04:00 阿鲁巴，百慕大，波多黎各，圭亚那'],
		['BOT4','UTC-04:00 玻利维亚'],
		...
		['EET-2EEST-3,M3.5.0/3,M10.5.0/4','UTC+02:00 南非，以色列'],
		...
		['UTC-7','UTC+07:00 泰国'],
		['KRAT-7KRAST,M3.5.0,M10.5.0/3','UTC+07:00 俄罗斯，克拉斯诺亚尔斯克'],
		['UTC-8','UTC+08:00 台湾, 中国, 香港, 澳洲西部, 新加坡, 俄罗斯'],
		...
		['UTC-12','UTC+12:00 斐济'],
		['NZST-12NZDT,M9.5.0/2,M4.1.0/3','UTC+12:00 新西兰']
	], value: nvram.tm_sel },
	...
	null,
	{ title: '自动时间同步', name: 'ntp_updates', type: 'select', options: [[-1,'不同步'],[0,'启动时更新'],[1,'１小时'],[2,'２小时'],[4,'４小时'],[6,'６小时'],[8,'８小时'],[12,'１２小时'],[24,'每天一次']],
		value: nvram.ntp_updates },
	{ title: '需要时同步', indent: 2, name: 'f_ntp_tdod', type: 'checkbox', value: nvram.ntp_tdod != '0' },
	{ title: 'NTP网络时间服务器', name: 'f_ntp_server', type: 'select', options: ntpList, value: ntpSel },

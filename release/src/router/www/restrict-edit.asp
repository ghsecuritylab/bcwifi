...
var ipp2p = [
	[0,'IPP2P (禁用)'],[0xFFFF,'所有'],[1,'AppleJuice'],[2,'Ares'],[4,'BitTorrent'],[8,'Direct Connect'],
...
function save()
{
	...
	if (E('rt_norm').checked) {
		...
		if (e.value != 0) {
			a = cg.getAllData();
			if (a.length == 0) {
				ferror.set(e, 'MAC 错误 或 IP 地址 已被指定', 0);
				return;
			}
	...
	if (data.length >= 2048) {
		alert('规则太长，请减少 ' + (data.length - 2048) + ' 字符.');
		return;
	}
...
createFieldTable('', [
	...
	{ title: '应用于', name: 'f_comp_all', type: 'select', options: [[0,'所有电脑/设备'],[1,'如下电脑/设备'],[2,'除了如下电脑/设备']], value: 0 },
	{ title: '&nbsp;', text: '<table class="tomato-grid" cellspacing=1 id="res-comp-grid"></table>' },
	{ title: '阻止资源', name: 'f_block_all', type: 'checkbox', suffix: ' 阻止所有的因特网访问', value: 0 },
	{ title: '端口/<br>应用', indent: 2, text: '<table class="tomato-grid" cellspacing=1 id="res-bp-grid"></table>' },
...
<tr><td id='footer' colspan=2>
	...
	<input type='button' value='删除...' id='delete-button' onclick='remove()'>
	&nbsp;

...
dg.populate = function()
{
	...
	for (i = list.length - 1; i >= 0; --i) {
		...
		if (e.mac.match(/^(..):(..):(..)/)) {
			b += '<br><small>' +
				'<a href="http://standards.ieee.org/cgi-bin/ouisearch?' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="OUI 检索">[oui]</a> ' +
				'<a href="javascript:addStatic(' + i + ')" title="加入静态DHCP租用...">[绑定]</a> ' +
...
dg.setup = function()
{
	...
	this.headerSet(['接口', 'MAC 地址', 'IP 地址', '名称', '信号强度', '信号质量', 'TX/RX 速率', '剩余租约']);
	this.populate();
	this.sort(2);
}
...
<div class='section'>
	<table id='dev-grid' class='table table-bordered table-striped' cellspacing=0></table>

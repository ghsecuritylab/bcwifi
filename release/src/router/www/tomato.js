	...
	submit: function(fom, async, url) {
		...
		for (var i = 0; i < fom.elements.length; ++i) {
			...
			if (f.name == '_nextwait') {
		        ...
		        v.push(escapeCGI(f.name) + '=' + encodeURI(f.value));
		}
...
function _v_range(e, quiet, min, max, name)
{
	...
	if ((!v.match(/^ *[-\+]?\d+ *$/)) || (v < min) || (v > max)) {
		ferror.set(e, '不正确的 ' + name + '. 有效范围为: ' + min + '-' + max, quiet);
		return 0;
	}
	e.value = v * 1;
	ferror.clear(e);
...
function v_macip(e, quiet, bok, lan_ipaddr, lan_netmask)
{
	...
	for (i = 0; i < a.length; ++i) {
		...
		if ((aton(b) & aton(lan_netmask))!=(aton(lan_ipaddr) & aton(lan_netmask))) {
			ferror.set(e, 'IP 地址 超出 LAN', quiet);
			return false;
		}
...
function v_dns(e, quiet)
{
	...
	if (e.value == '') {
		...
		if (s.length == 1) {
			...
			ferror.set(e, '无效的 IP 地址或 端口', quiet);
			return false;
		}
...
function _v_iptip(e, ip, quiet)
{
	...
	if (ip.match(/^(.*)-(.*)$/)) {
		...
		b = fixIP(RegExp.$2);
		if ((a == null) || (b == null)) {
			ferror.set(e, oip + ' - 不正确的 IP 地址范围', quiet);
			return null;
		}
		ferror.clear(e);
	...
	ip = fixIP(ip);
	if (!ip) {
		ferror.set(e, oip + ' - 无效的IP 地址', quiet);
		return null;
	}
...
function _v_domain(e, dom, quiet)
{
	...
	if (s.length > 0) {
		...
		if (s == null) {
			ferror.set(e, "无效的名称. 仅 \"A-Z 0-9 . -\" 被允许.", quiet);
			return null;
		}
	}
	ferror.clear(e);
...
function _v_ipv6_addr(e, ip, ipt, quiet)
{
	...
	if ((ipt) && ip.match(/^([A-Fa-f0-9:]+)\/(\d+)$/)) {
		...
		if (b < 0 || b > 128) {
			ferror.set(e, oip + ' - IPv6地址上的CIDR标记无效', quiet);
			return null;
		}
		ferror.clear(e);
...
function v_iptport(e, quiet)
{
	...
	if (a.length > 10) {
		ferror.set(e, '您输入的端口列表或端口范围超出10个，请减少一些端口.', quiet);
		return 0;
	}
...
function v_mac(e, quiet)
{
	...
	mac = fixMAC(e.value);
	if ((!mac) || (isMAC0(mac))) {
		ferror.set(e, '不正确的 MAC 地址', quiet);
		return 0;
	}
	e.value = mac;
	ferror.clear(e);
...
function v_length(e, quiet, min, max)
{
	...
	if (n < min) {
		ferror.set(e, '不正确的长度. 请至少输入 ' + min + ' 个字符' + (min == 1 ? '.' : 's.'), quiet);
		return 0;
	}
	max = max || e.maxlength;
	if (n > max) {
		ferror.set(e, '不正确的长度. 请缩短长度少于 ' + max + ' 个字符', quiet);
		return 0;
	}
	e.value = s;
	ferror.clear(e);
...
/* IPV6-END */
				...
				if ((t = _v_iptip(e, v[i], 1)) == null) {
					ferror.set(e, e._error_msg + ', 或无效的域', quiet);
					return 0;
				}
/* IPV6-BEGIN */
			}
	...
	for (i = 0; i < v.length; ++i) {
		...
		if (s.length > 0) {
			...
			if (s.search(re) == -1 || s.search(/^\d+$/) != -1) {
				ferror.set(e, '无效的主机名. 仅 "A-Z 0-9" 和"-" 被允许 (最多63个字符).', quiet);
				return null;
			}
		} else if (required) {
			ferror.set(e, '无效的主机名', quiet);
...
function v_nodelim(e, quiet, name, checklist)
{
	...
	if (e.value.indexOf('<') != -1 ||
	   (checklist && e.value.indexOf('>') != -1)) {
		ferror.set(e, '不正确的 ' + name + ': \"<\" ' + (checklist ? '或 \">\" 是' : '是') + ' 不允许的.', quiet);
		return 0;
	}
	ferror.clear(e);
	return 1;
...
function _tabCreate(tabs)
{
	...
	for (var i = 0; i < arguments.length; ++i)
		buf.push('<li  id="' + arguments[i][0] + '"><a href="javascript:tabSelect(\'' + arguments[i][0] + '\')">' + arguments[i][1] + '</a>');
...
function tabHigh(id)
{
	var a = E('tabs').getElementsByTagName('LI');
	...
	for (i = 0; i < menu.length; ++i) {
		...
		if (m.length == 2) {
			...
			for (j = 0; j < m[3].length; ++j) {
				...
				a = m[1] + '-' + sm[1];
				if (a == 'status-overview.asp') a = '/';
				buf.push('<a href="' + a + '" class="indent2' + (((on1) && (name == sm[1])) ? ' active2' : '') + '">' + sm[0] + '</a>');
			}
		}
	}
...
function reboot()
{
	if (confirm("是否重启?")) form.submitHidden('tomato.cgi', { _reboot: 1, _commit: 0, _nvset: 0 });
}

function shutdown()
{
	if (confirm("是否关机?")) form.submitHidden('shutdown.cgi', { });
}

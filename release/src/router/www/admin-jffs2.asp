...
function formatClicked()
{
	...
	if (!confirm("确认要格式化JFFS分区?")) return;
	save(1);
}
...
createFieldTable('', [
	...
	null,
	{ title: '总容量/可用空间', text: (((jffs2.mnt) || (jffs2.size > 0)) ? scaleSize(jffs2.size) : '') + ((jffs2.mnt) ? ' / ' + scaleSize(jffs2.free) : ' (not mounted)') },
	null,
	{ title: '', custom: '<input type="button" value="格式化 / 清除..." onclick="formatClicked()" id="format"><br><br>' +

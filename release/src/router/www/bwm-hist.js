...
function checkRstats()
{
	if (nvram.rstats_enable != '1') {
		...
		W('<div class="note-warning">程序繁忙或没有响应. 请稍后再试.</div>');
	}
}
...
function checkCstats()
{
	if (nvram.cstats_enable != '1') {
		W('<div class="note-disabled">IP流量监控已禁用.</b><br><br><a href="admin-iptraffic.asp">开启 &raquo;</a></div>');
		E('cstats').style.display = 'none';
	}
	else if (cstats_busy) {
		W('<div class="note-warning">IP流量监控程序cstat没有响应或正忙，请几秒钟后重新加载.</div>');
	}
}

...
function verifyFields(focused, quiet)
{
	...
	if (!E('_f_pptp_client_dfltroute').checked && !v_ip('_pptp_client_srvsub', true)) { ferror.set(E('_pptp_client_srvsub'), "子网错误.", quiet); ret = 0; }
	if (!E('_f_pptp_client_dfltroute').checked && !v_ip('_pptp_client_srvsubmsk', true)) { ferror.set(E('_pptp_client_srvsubmsk'), "掩码错误.", quiet); ret = 0; }
...
createFieldTable('', [
	...
	{ title: '接受DNS配置', name: 'pptp_client_peerdns', type: 'select', options: [[0, '禁用'],[1, '启用'],[2, '独占']], value: nvram.pptp_client_peerdns },
	...
	{ title: '创建隧道NAT', name: 'f_pptp_client_nat', type: 'checkbox', value: nvram.pptp_client_nat != 0 },
	{ title: 'MTU设置', multi: [
    ...
    W('<input type="button" value="' + (pptpup ? '停止' : '立即启动') + ' Now" onclick="toggle(\'pptpclient\', pptpup)" id="_pptpclient_button">');
</script>
</div>

...
utf2gbk(char *utf, char *gbk)
{
	...
	if(!nls)
	{
		...
		len = 0;
		if (ret=utf8_mbstowcs(unibuf, utf, strlen(utf)))
		{
			int i;
			for (i = 0; (i < ret) && unibuf[i]; i++) {
				int charlen;
				charlen = nls->uni2char(unibuf[i], &gbk[len], NLS_MAX_CHARSET_SIZE);
				if (charlen > 0) {
					len += charlen;
				}
				else {
...
nvram_set(const char *name, const char *value)
{
	...
	//printk("nvram_set: name = %s, value = %s!\n", name, value);
		...
		if (strcmp(name, "wl0_ssid") == 0) 
		{
			char *gbk = tmpbuf;

			utf2gbk(value, gbk);		
			_nvram_set("wl0_ssid_gbk", gbk);		

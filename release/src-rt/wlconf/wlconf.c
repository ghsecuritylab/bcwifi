...
wlconf(char *name)
{
	...
	unsigned int i;
	char eaddr[32];
	int ap, apsta, wds, sta = 0, wet = 0, mac_spoof = 0, wmf = 0 , utf8 = 0;
	...
	if (wl_ap_build) {
		...
		for (i = 0; i < bclist->count; i++) {
			bsscfg = &bclist->bsscfgs[i];
			strcat_r(bsscfg->prefix, "encoding", tmp);
			if (strcmp(nvram_safe_get(tmp), "gbk") == 0) {
				utf8 = 0;
			}else {
				utf8 = 1;
			}
			...
			if (utf8) {
				...
				strcat_r(bsscfg->prefix, "ssid_gbk", tmp);
				ssid.SSID_len = strlen(nvram_safe_get(tmp));
				if (ssid.SSID_len > sizeof(ssid.SSID))
					ssid.SSID_len = sizeof(ssid.SSID);
				strncpy((char *)ssid.SSID, nvram_safe_get(tmp), ssid.SSID_len);
